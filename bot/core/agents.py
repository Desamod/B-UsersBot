...
def set_user_agent(session_name: str):
    user_agents = {}
    with open('bot/config/User-Agents.json', encoding="utf-8-sig") as file:
        user_agents = json.load(file)

    if session_name in user_agents:
        return user_agents[session_name]
    else:
        with open('bot/config/User-Agents.json', "w", encoding="utf-8-sig") as file:
            user_agent = generate_random_user_agent(device_type='android', browser_type='chrome')
            user_agents[session_name] = user_agent
            file.write(json.dumps(user_agents, indent=4))
            return user_agent
